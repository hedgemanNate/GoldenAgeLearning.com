...
function BookingFlowContent({ params }: { params: Promise<{ classId: string }> }) {
  ...
  const loginHref = `/auth/login?redirectTo=${encodeURIComponent(`/book/${resolvedParams.classId}?resumeBooking=1&step=4`)}`;
  ...
  useEffect(() => {
    ...
    const shouldResumeBooking = searchParams.get("resumeBooking") === "1";
    const resumeStep = searchParams.get("step");

    if (shouldResumeBooking && resumeStep === "4") {
      setCurrentStep(4);
      setStep2Error("");
      setStep3Error("");
      setBookingError("");
      ...
      <div className="flex-grow max-w-2xl mx-auto w-full px-[24px] pt-[120px] pb-[32px] md:pt-[140px] md:pb-[48px]">
        ...
        {currentStep === 2 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-[rgba(201,168,76,0.08)] border border-[rgba(201,168,76,0.25)] rounded-[10px] px-[18px] py-[18px] mb-[32px]">
              <h2 className="font-sans text-[20px] font-semibold text-[var(--color-cream)] mb-[8px]">
                Have you taken a class with us before?
              </h2>
              <p className="font-sans text-[16px] leading-[1.6] text-[rgba(245,237,214,0.72)] mb-[16px]">
                If you have, you can sign in with your account and we will bring you right back to this class so you can finish booking without starting over.
              </p>
              <Link
                href={loginHref}
                className="inline-flex min-h-[52px] items-center justify-center rounded-[8px] border border-[var(--color-gold)] px-[18px] text-[16px] font-medium text-[var(--color-gold)] hover:bg-[rgba(201,168,76,0.1)] transition-all"
              >
                I have taken a class before — sign in here
              </Link>
            </div>
