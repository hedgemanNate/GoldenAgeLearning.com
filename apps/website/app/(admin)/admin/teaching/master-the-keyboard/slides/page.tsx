...
import { masterTheKeyboardSlides } from "../_slides/content";
...
export default function MasterTheKeyboardSlidesLaunch() {
  ...
  // No session yet, or it has ended → offer to start a fresh one.
  if (!isActive) {
    return (
      <Shell>
        <h2
  ...
  // Session is active — show controls + display URL.
  const displayUrl =
  ...
  return (
    <Shell>
        ...
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            <button
              onClick={next}
              disabled={
                (session?.currentSlide ?? 0) >= TOTAL_SLIDES - 1
              }
              style={navBtn(
                (session?.currentSlide ?? 0) >= TOTAL_SLIDES - 1
              )}
      ...
      {/* Quick links to other teaching pages (overlay also lives on these) */}
      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
