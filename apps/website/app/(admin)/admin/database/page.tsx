  ...
  const typedEditorConfigs = useMemo<Record<CollectionKey, TypedEditorConfig | null>>(() => ({
    users: {
      ...
      toRecord: (values, base) => ({
        ...
        role: String(base.role ?? "customer"),
    ...
    try {
      ...
      switch (selectedCollection) {
        case "users": {
          ...
          const existingRole = selectedRecord?.raw?.role;
          const nextRole = (parsed as { role?: unknown }).role;

          if (isCreating) {
            if (nextRole !== "customer") {
              throw new Error("New user records created here must start as customer. Change roles from the Staff page.");
            }
          } else if (existingRole !== nextRole) {
            throw new Error("Role changes are disabled in the database workspace. Use the Staff page instead.");
          }
