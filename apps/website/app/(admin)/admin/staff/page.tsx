...
export default function AdminStaff() {
  ...
  async function performRoleChange(uid: string, newRole: "customer" | StaffRole) {
  ...
  async function changeRole(uid: string, newRole: StaffRole) {
    await performRoleChange(uid, newRole);
  }
  ...
  return (
    <div className="p-[32px] font-sans">
      ...
      {actionError && (
        <div className="mb-[20px] rounded-[8px] border border-[rgba(220,38,38,0.24)] bg-[rgba(220,38,38,0.08)] px-[16px] py-[12px] text-[13px] text-[#FCA5A5]">
          {actionError}
        </div>
      )}
              ...
              <div className="flex items-center gap-[10px] flex-shrink-0">
                {member.role === "staff" ? (
                  <button
                    ...
                    disabled={submittingUid === member.uid}
                  ...
                  >
                    {submittingUid === member.uid ? "Saving…" : "Make Super Admin"}
                  </button>
                ) : (
                  <button
                    ...
                    className="text-[12px] text-[rgba(245,237,214,0.45)] hover:text-[var(--color-cream)] border border-[rgba(245,237,214,0.1)] px-[12px] py-[6px] rounded-[6px] transition"
                  >
                    {submittingUid === member.uid ? "Saving…" : "Make Staff"}
                  </button>
                )}
                <button
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-[16px]" onClick={(e) => { if (e.target === e.currentTarget) setDeleteModal(false); }}>
          <div className="bg-[var(--color-dark-surface)] rounded-[12px] w-full max-w-[400px] px-[28px] py-[28px]">
            ...
            <div className="flex justify-end gap-[10px]">
              ...
              <button
                onClick={async () => {
                  await performRoleChange(deleteTarget.uid, "customer");
                  setDeleteModal(false);
                }}
                disabled={submittingUid === deleteTarget.uid}
              ...
              >
                {submittingUid === deleteTarget.uid ? "Removing…" : "Remove Access"}
              </button>
            </div>
          </div>
        </div>
