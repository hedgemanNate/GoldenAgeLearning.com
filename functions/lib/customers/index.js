...
exports.onCustomerCreated = functions.auth.user().onCreate(async (user) => {
    ...
    if (!email)
        return; // No email on file — SMS handled separately
    ...
    const name = (userSnap.exists() ? userSnap.val().name : null) ??
        user.displayName ??
        "Friend";
    await (0, emails_1.sendWelcomeEmail)(email, { customerName: name });
});
